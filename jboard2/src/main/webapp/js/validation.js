...
window.onload = function(){
        ...
        fetch('/jboard2/user/checkUser.do?type=email&value=' + value)
            ...
            .then((data) => {
                ...
                if (data.result > 0) {
                    ...
                    resultEmail.innerText = '이메일 인증코드 6자리 입력';
                }
            }).catch((err) => {
                console.log(err);
            });
    ...
    btnAuthEmail.onclick = function () {
        ...
        fetch('/jboard2/user/checkUser.do?type=auth&value=' + value, {
            method: 'POST',
            ...
            .then((data) => {
                ...
                if (data.result > 0) {
                    ...
                    resultEmail.innerText = '이메일 인증에 성공했습니다.';
                    resultEmail.style.color = 'green';
                    isEmailOk = true;
                    // 여기에서 인증 성공 시 처리를 수행할 수 있음
                } else {
                    resultEmail.innerText = '이메일 인증에 실패했습니다.';
                    resultEmail.style.color = 'red';
                    isEmailOk = false;
                    // 여기에서 인증 실패 시 처리를 수행할 수 있음
                }
            }).catch((err) => {
                console.log(err);
            });
    ...
    btnCheckHp.onclick = function () {
        ...
        // 유효성 검사
        if (!value.match(reHp)) {
            resultHp.innerText = '휴대폰 번호 형식이 맞지 않습니다.';
